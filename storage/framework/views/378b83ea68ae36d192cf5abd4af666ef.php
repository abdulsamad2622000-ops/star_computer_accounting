<?php $__env->startSection('title', 'Sale Point'); ?>
...
<div class="receipt">
    <div class="header">
        <div class="brand-block">
            <h1><?php echo e($s->business_name ?? 'STAR COMPUTER'); ?></h1>
            <div class="subtitle"><strong><?php echo e($s->tagline ?? ''); ?></strong></div>
            <div class="contact-grid">
                <?php $__currentLoopData = $contacts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $contact): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="contact-card">
                    <div class="cname"><?php echo e($contact->name); ?></div>
            ...
            <div style="font-size:11px;color:#3e5a7a;margin-top:6px">
                <strong>NTN:</strong> <?php echo e($s->ntn); ?>
        ...
        <div>
            ...
            <div class="bank-block" style="margin-bottom:6px">
                <div class="bank-title"><?php echo e($bank->bank_name); ?> — <?php echo e($bank->account_title); ?></div>
                <div class="bank-row">
                    <div class="bank-label">Account Title</div>
                    <div class="bank-value"><?php echo e($bank->account_title); ?></div>
                </div>
                <div class="bank-row">
                    <div class="bank-label">Account Number</div>
                    <div class="bank-value"><?php echo e($bank->account_number); ?></div>
                </div>
                <?php if($bank->iban): ?>
                <div class="bank-row">
                    <div class="bank-label">IBAN</div>
                    <div class="bank-value"><?php echo e($bank->iban); ?></div>
                </div>
                <?php endif; ?>
                <?php if($bank->qr_code): ?>
    ...
    <div class="meta">
        ...
        <div class="field">
            <label>Date</label>
            <input type="date" id="sale_date" value="<?php echo e(date('Y-m-d')); ?>">
        </div>
        ...
        <div class="field">
            <label>Mobile</label>
            <input type="text" id="customer_mobile" readonly placeholder="Auto">
        </div>
        <div class="field">
            <label>CNIC</label>
            <input type="text" id="customer_cnic" readonly placeholder="Auto">
        </div>
        <div class="field">
            <label>Staff *</label>
            <select id="user_id">
    ...
    <div class="totals">
        ...
        <div class="block">
            ...
            <div class="trow">
                <div class="tlabel">Reference</div>
    ...
    <div class="note-box">
        ...
        <?php $__currentLoopData = explode("\n", $s->notes); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $note): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php if(trim($note)): ?>• <?php echo e(trim($note)); ?><br><?php endif; ?>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </div>
    <?php endif; ?>

    <div class="footer-box">
        <div><?php echo e($s->thank_you_message ?? 'Thank you for your business.'); ?></div>
        <div>Powered by STAR COMPUTER POS</div>
    </div>

    <div style="display:grid;grid-template-columns:1fr 130px;gap:10px;margin-top:12px">
        <button class="submit-btn" id="submitBtn" onclick="submitSale()">✅ Submit Bill</button>
    ...
    <div id="saleAlert" class="sale-alert"></div>
</div>

<!-- Sale History -->
<div class="history-section">
    <div class="history-header"><h6>🕐 Sale History</h6></div>
    <div class="filter-bar">
        <input type="text" id="search_memo" placeholder="🔍 Memo #..." oninput="filterHistory()">
        <input type="text" id="search_customer" placeholder="🔍 Customer..." oninput="filterHistory()">
        <input type="date" id="search_from" value="<?php echo e(date('Y-m-d')); ?>" onchange="filterHistory()">
        <input type="date" id="search_to" value="<?php echo e(date('Y-m-d')); ?>" onchange="filterHistory()">
    ...
    <table class="history-table">
        ...
        <tbody id="historyBody">
            ...
            <tr>
                <td><span class="memo-badge"><?php echo e($sale->memo_no); ?></span></td>
                <td><?php echo e(\Carbon\Carbon::parse($sale->date)->format('d-m-Y')); ?></td>
                <td><?php echo e($sale->customer->name ?? '—'); ?></td>
                <td><?php echo e($sale->customer->contact1 ?? '—'); ?></td>
                <td><?php echo e($sale->salesperson->name ?? '—'); ?></td>
                <td>Rs. <?php echo e(number_format($sale->total)); ?></td>
                <td>
                    <span class="type-badge type-<?php echo e($sale->payment_type); ?>">
                ...
                <td style="display:flex;gap:4px;align-items:center;flex-wrap:wrap">
                    <button class="print-btn"
                        onclick="openTransferModal(<?php echo e($sale->id); ?>, '<?php echo e($sale->memo_no); ?>')">
                        🔄 Transfer
                    </button>
                    <button class="print-btn"
                        onclick="openSaleReturn(<?php echo e($sale->id); ?>, '<?php echo e($sale->memo_no); ?>')">
                        ↩️ Return
                    </button>
                    <button class="print-btn"
                        onclick="window.open('/sales/<?php echo e($sale->id); ?>/invoice','_blank')">
                        🖨️ Print
                    </button>
                    <a href="/sales/<?php echo e($sale->id); ?>/invoice/pdf"
                       class="print-btn" target="_blank" style="text-decoration:none">
                        📄 PDF
                    </a>
                </td>
            </tr>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content" style="border-radius:12px;border:none">
            <div class="modal-header border-0 pb-0">
                <h6 class="modal-title fw-bold">↩️ Sale Return — Memo# <span id="returnMemoNo"></span></h6>
            ...
            <div class="modal-body">
                <div class="row g-3 mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="date" id="returnDate" class="form-control" value="<?php echo e(date('Y-m-d')); ?>">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Reason <small class="text-muted">(Optional)</small></label>
                        <input type="text" id="returnReason" class="form-control" placeholder="e.g. Defective product">
                    </div>
                </div>
                <div id="returnItemsBody">
            ...
            <div class="modal-footer border-0">
                ...
                <button type="button" class="btn btn-warning btn-sm" id="submitReturnBtn" onclick="submitSaleReturn()">
                    ↩️ Process Return
                </button>
            </div>
        </div>
...
<div class="modal fade" id="transferModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content" style="border-radius:12px;border:none">
            <div class="modal-header border-0 pb-0">
                <h6 class="modal-title fw-bold">🔄 Transfer Bill — Memo# <span id="transferMemoNo"></span></h6>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <div class="alert alert-warning" style="font-size:13px">
                    ⚠️ Yeh bill kisi aur customer ke naam transfer hoga. Old customer ki ledger update ho jayegi.
                </div>
                <div class="mb-3">
                    <label class="form-label fw-bold">New Customer Select Karein *</label>
                    <div style="position:relative">
                        <input type="text" id="transfer_customer_search"
                               class="form-control"
                               placeholder="Type customer name..."
                               autocomplete="off"
                               oninput="searchTransferCustomer(this.value)">
                        <div id="transfer_customer_list"
                             style="display:none;position:absolute;top:100%;left:0;right:0;
                                    background:#fff;border:1px solid #5a7ca8;border-radius:6px;
                                    max-height:180px;overflow-y:auto;z-index:999;
                                    box-shadow:0 4px 12px rgba(0,0,0,.1)"></div>
                    </div>
                    <input type="hidden" id="transfer_customer_id">
                </div>
            </div>
            <div class="modal-footer border-0">
                <button type="button" class="btn btn-outline-secondary btn-sm" data-bs-dismiss="modal">Cancel</button>
...
let currentTransferSaleId = null;
...
function searchCustomer(query) {
    ...
    if (!query) { list.style.display = 'none'; document.getElementById('customer_id').value = ''; return; }
    ...
    list.innerHTML = filtered.map(c => `
        <div onclick="selectCustomer(${c.id},'${c.name}','${c.contact1 ?? ''}','${c.cnic ?? ''}')"
...
function searchTransferCustomer(query) {
    ...
    if (!filtered.length) { list.style.display = 'none'; return; }
    list.innerHTML = filtered.map(c => `
        <div onclick="selectTransferCustomer(${c.id},'${c.name}')"
             style="padding:7px 10px;font-size:12px;cursor:pointer;border-bottom:1px solid #f0f0f0;color:#0e2a4f"
             onmouseover="this.style.background='#e7f1ff'"
             onmouseout="this.style.background='#fff'">
            <strong>${c.name}</strong>
...
function selectTransferCustomer(id, name) {
    document.getElementById('transfer_customer_id').value     = id;
    document.getElementById('transfer_customer_search').value = name;
    document.getElementById('transfer_customer_list').style.display = 'none';
}

document.addEventListener('click', function(e) {
    ['customer_list','transfer_customer_list'].forEach(id => {
    ...
    list.innerHTML = filtered.map(p => `
        <div class="autocomplete-item" onclick='selectProduct(${JSON.stringify(p)},${rowId})'>
            ...
            <span style="color:#22c55e;float:right">
                Stock: ${p.remaining_qty} | Rate: Rs.${Number(p.sale_price).toLocaleString()}
            </span>
        </div>`).join('');
    list.style.display = 'block';
}
...
function selectProduct(product, rowId) {
    ...
    document.getElementById(`rate_${rowId}`).value = product.sale_price || 0;
...
function recalcRow(rowId) {
    ...
    document.getElementById(`amt_${rowId}`).value = (qty * rate).toFixed(2);
    recalcTotals();
}
...
function getRowTemplate(rid, sn) {
    return `
        <tr id="row_${rid}">
            ...
            <td><input type="text" id="code_${rid}" placeholder="Auto"></td>
            <td><input type="number" id="qty_${rid}" value="1" min="1" oninput="recalcRow(${rid})"></td>
            <td><input type="number" id="rate_${rid}" value="0" min="0" step="0.01" oninput="recalcRow(${rid})"></td>
            <td><input type="number" id="amt_${rid}" value="0" readonly></td>
            <td class="no-print">
...
function printLastBill() {
    if (!lastSaleId) { alert('Pehle sale submit karein!'); return; }
    ...
    iframe.src = `/sales/${lastSaleId}/invoice`;
...
async function submitSale() {
    ...
    const discount   = parseFloat(document.getElementById('discountAmount')?.value) || 0;
    const paid       = parseFloat(document.getElementById('paidAmount')?.value)     || 0;
    const payType    = document.getElementById('payment_type')?.value || 'cash';

    // ✅ Strict validation
    if (!customerId || customerId === '') {
        alert('❌ Customer select karna zaroori hai!');
        document.getElementById('customer_search').focus();
        return;
    }
    if (!userId) {
        alert('❌ Staff select karein!');
        return;
    }
    if (!date) {
    ...
    document.querySelectorAll('#itemsBody tr').forEach(row => {
        ...
        const rate = parseFloat(document.getElementById(`rate_${rid}`)?.value || 0);
        ...
        const product = allProducts.find(p =>
            ...
            (code && p.stock_code && p.stock_code.toLowerCase() === code.toLowerCase())
        );

        if (!product) {
            alert(`⚠️ "${name}" product inventory mein nahi mila!`);
            hasItems = false;
            return;
        }
        ...
        items.push({
            ...
            stock_code:  code || product.stock_code || null,
            qty,
            rate,
    ...
    try {
        const res = await fetch('<?php echo e(route("sales.store")); ?>', {
            method: 'POST',
            headers: {
        ...
        if (data.success) {
            ...
            document.getElementById('saleAlert').innerHTML = `
                <div style="background:#f0fdf4;color:#166534;padding:10px 14px;border-radius:8px;
                            font-weight:600;margin-top:8px;display:flex;align-items:center;
                            gap:10px;flex-wrap:wrap">
                    <span>✅ Bill ban gaya! Memo: <strong>${data.memo_no}</strong></span>
                    <a href="/sales/${data.sale_id}/invoice" target="_blank"
                       style="background:#163a6f;color:#fff;padding:5px 14px;border-radius:6px;
                              text-decoration:none;font-size:12px;font-weight:700">🖨️ Print</a>
                    <a href="/sales/${data.sale_id}/invoice/pdf" target="_blank"
                       style="background:#5a7ca8;color:#fff;padding:5px 14px;border-radius:6px;
                              text-decoration:none;font-size:12px;font-weight:700">📄 PDF</a>
                </div>`;
            ...
            document.getElementById('saleAlert').innerHTML = `
                <div style="background:#fef2f2;color:#ef4444;padding:10px 14px;border-radius:8px;
                            font-weight:600;margin-top:8px">❌ ${data.message}</div>`;
        }

    } catch(e) {
        document.getElementById('saleAlert').innerHTML = `
            <div style="background:#fef2f2;color:#ef4444;padding:10px 14px;border-radius:8px;
                        font-weight:600;margin-top:8px">❌ Server error! Dobara try karein.</div>`;
    }
...
async function filterHistory() {
    ...
    const params = new URLSearchParams({
        ...
        customer_name: document.getElementById('search_customer').value,
        from, to,
    });

    const res   = await fetch(`<?php echo e(route('sales.history')); ?>?${params}`);
...
function renderHistory(sales) {
    ...
    if (!sales.length) {
        tbody.innerHTML = `<tr><td colspan="8" style="text-align:center;padding:20px;color:#6b7280">Koi sale nahi mili</td></tr>`;
        return;
    }
    tbody.innerHTML = sales.map(s => `
        <tr>
            <td><span class="memo-badge">${s.memo_no}</span></td>
            <td>${s.date}</td>
            <td>${s.customer?.name ?? '—'}</td>
            <td>${s.customer?.contact1 ?? '—'}</td>
            <td>${s.salesperson?.name ?? '—'}</td>
            <td>Rs. ${Number(s.total).toLocaleString()}</td>
            <td><span class="type-badge type-${s.payment_type}">${s.payment_type}</span></td>
            <td style="display:flex;gap:4px;align-items:center;flex-wrap:wrap">
                <button class="print-btn" onclick="openTransferModal(${s.id},'${s.memo_no}')">🔄 Transfer</button>
                <button class="print-btn" onclick="openSaleReturn(${s.id},'${s.memo_no}')">↩️ Return</button>
                <button class="print-btn" onclick="window.open('/sales/${s.id}/invoice','_blank')">🖨️ Print</button>
                <a href="/sales/${s.id}/invoice/pdf" class="print-btn" target="_blank" style="text-decoration:none;display:inline-block">📄 PDF</a>
            </td>
        </tr>`).join('');
}
function setToday() {
...
function clearFilter() {
    ...
    // ✅ All button = explicitly sab load karo
    const params = new URLSearchParams({ all: '1' });
    fetch(`<?php echo e(route('sales.history')); ?>?${params}`)
        .then(r => r.json())
        .then(sales => renderHistory(sales));
}

// ── Sale Return ──────────────────────────────────────────
async function openSaleReturn(saleId, memoNo) {
    currentReturnSaleId = saleId;
    document.getElementById('returnMemoNo').textContent = memoNo;
    document.getElementById('returnItemsBody').innerHTML = `<div class="text-center py-3"><div class="spinner-border spinner-border-sm"></div> Loading...</div>`;
    new bootstrap.Modal(document.getElementById('saleReturnModal')).show();

    const res  = await fetch(`/sales/${saleId}/return-items`);
    const data = await res.json();

    if (!data.items || !data.items.length) {
        document.getElementById('returnItemsBody').innerHTML = `<div class="alert alert-warning">Koi returnable item nahi hai!</div>`;
        return;
    }
    ...
    data.items.forEach(item => {
        html += `<tr>
            ...
            <td><input type="number" class="form-control form-control-sm" id="return_qty_${item.id}" value="0" min="0" max="${item.returnable_qty}" style="width:80px"></td>
...
async function submitSaleReturn() {
    const items = [];
    document.querySelectorAll('#returnItemsBody input[id^="return_qty_"]').forEach(input => {
        const qty = parseInt(input.value || 0);
        if (qty > 0) items.push({ id: input.id.replace('return_qty_', ''), qty });
    });
    ...
    try {
        const res  = await fetch(`/sales/${currentReturnSaleId}/return`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content },
            body: JSON.stringify({ items, date: document.getElementById('returnDate').value, reason: document.getElementById('returnReason').value })
        });
        const data = await res.json();
        if (data.success) {
            bootstrap.Modal.getInstance(document.getElementById('saleReturnModal')).hide();
            alert('✅ Return process ho gaya!');
            filterHistory();
...
// ── Transfer ─────────────────────────────────────────────
function openTransferModal(saleId, memoNo) {
    currentTransferSaleId = saleId;
    document.getElementById('transferMemoNo').textContent = memoNo;
    document.getElementById('transfer_customer_search').value = '';
    document.getElementById('transfer_customer_id').value     = '';
    document.getElementById('transfer_customer_list').style.display = 'none';
    new bootstrap.Modal(document.getElementById('transferModal')).show();
}

async function submitTransfer() {
    const customerId = document.getElementById('transfer_customer_id').value;
    ...
    try {
        const res  = await fetch(`/sales/${currentTransferSaleId}/transfer`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content },
            body: JSON.stringify({ new_customer_id: customerId })
        });
        const data = await res.json();
        if (data.success) {
            bootstrap.Modal.getInstance(document.getElementById('transferModal')).hide();
            alert('✅ Bill transfer ho gaya!');
            filterHistory();
        } else { alert('❌ ' + data.message); }
    } catch(e) { alert('❌ Server error!'); }

    btn.disabled = false; btn.textContent = '🔄 Transfer Bill';
}
</script>
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\star_computer_accounting\resources\views/sales/pos.blade.php ENDPATH**/ ?>