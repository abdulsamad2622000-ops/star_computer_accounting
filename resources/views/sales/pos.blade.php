...
<div class="receipt">
    ...
    <div class="meta">
        ...
        <div class="field">
            <label>Date</label>
            <input type="date" id="sale_date" value="{{ date('Y-m-d') }}">
        </div>
...
<div class="history-section">
    ...
    <table class="history-table">
        ...
        <tbody id="historyBody">
            ...
            <tr>
                <td><span class="memo-badge">{{ $sale->memo_no }}</span></td>
                <td>{{ \Carbon\Carbon::parse($sale->date)->format('d-m-Y') }}</td>
                <td>{{ $sale->customer->name ?? '—' }}</td>
                <td>{{ $sale->customer->contact1 ?? '—' }}</td>
                <td>{{ $sale->salesperson->name ?? '—' }}</td>
                <td>Rs. {{ number_format($sale->total) }}</td>
                <td>
                    <span class="type-badge type-{{ $sale->payment_type }}">
                        {{ ucfirst($sale->payment_type) }}
                    </span>
                </td>
                <td style="display:flex;gap:4px;align-items:center;flex-wrap:wrap">
                    <button class="print-btn"
                        onclick="openTransferModal({{ $sale->id }}, '{{ $sale->memo_no }}')">
                        🔄 Transfer
                    </button>
                    <button class="print-btn"
                        onclick="openSaleReturn({{ $sale->id }}, '{{ $sale->memo_no }}')">
                        ↩️ Return
                    </button>
                    <button class="print-btn"
                    ...
                    <a href="/sales/{{ $sale->id }}/invoice/pdf"
...
async function filterHistory() {
    ...
    const res   = await fetch(`{{ route('sales.history') }}?${params}`);
...
function clearFilter() {
    ...
    fetch(`{{ route('sales.history') }}?${params}`)
        .then(r => r.json())
